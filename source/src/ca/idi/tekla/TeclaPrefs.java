...
public class TeclaPrefs extends PreferenceActivity implements
	...
	protected void onCreate(Bundle icicle) {
		...
		mPredictionPreferences
				.removePreference(findPreference("show_suggestions"));
		mPredictionPreferences
				.removePreference(findPreference("auto_complete"));
		mPreferenceScreen = (PreferenceScreen) findPreference("english_ime_settings");
		mPreferenceScreen
				.removePreference(findPreference("alternative_output_settings"));
	...
	private void init() {
		...
		mConfigureInputAdapter = (BaseAdapter) mConfigureInputScreen
				.getRootAdapter();
		...
		TeclaApp.dictation_event = setDictationEvent.getSharedPreferences()
				.getInt("set_dictation_event", 55);
		...
		TeclaApp.disconnect_event = setDisconnectEvent.getSharedPreferences()
				.getInt("set_disconnect_event", 65);

		setPasswordLaunch = (Preference) findPreference(Persistence.SET_PASSWORD);

		setPasswordLaunch.setDefaultValue("Tecla123");
		TeclaApp.password = setPasswordLaunch.getSharedPreferences().getString(
				Persistence.SET_PASSWORD, "Tecla123");
		...
		mSwitchJ1 = new SwitchPreference(
				(PreferenceScreen) findPreference(Persistence.PREF_SWITCH_J1),
		...
		mSwitchJ2 = new SwitchPreference(
				(PreferenceScreen) findPreference(Persistence.PREF_SWITCH_J2),
		...
		mSwitchJ3 = new SwitchPreference(
				(PreferenceScreen) findPreference(Persistence.PREF_SWITCH_J3),
		...
		mSwitchJ4 = new SwitchPreference(
				(PreferenceScreen) findPreference(Persistence.PREF_SWITCH_J4),
		...
		mSwitchE1 = new SwitchPreference(
				(PreferenceScreen) findPreference(Persistence.PREF_SWITCH_E1),
		...
		mSwitchE2 = new SwitchPreference(
				(PreferenceScreen) findPreference(Persistence.PREF_SWITCH_E2),
		...
		if (!(TeclaApp.getInstance().isVoiceInputSupported())) {
			if (mPrefVoiceInput.isChecked())
				mPrefVoiceInput.setChecked(false);
		...
		if (!mPrefConnectToShield.isChecked()
				&& !mPrefFullScreenSwitch.isChecked()) {
		...
		mPrefMorseTimeUnit.setSummary(TeclaApp.persistence.getMorseTimeUnit()
				+ " ms");
		refreshSwitchesSummary();

		// If Morse mode is disabled, also disable all of the other prefs in the
		// same category
		if (!mPrefMorse.isEnabled()
				|| (mPrefMorse.isEnabled() && !mPrefMorse.isChecked())) {
		...
		registerReceiver(mReceiver, new IntentFilter(
				SwitchEventProvider.ACTION_SHIELD_CONNECTED));
		registerReceiver(mReceiver, new IntentFilter(
				SwitchEventProvider.ACTION_SHIELD_DISCONNECTED));

		getPreferenceManager().getSharedPreferences()
				.registerOnSharedPreferenceChangeListener(TeclaPrefs.this);

		// DETERMINE WHICH PREFERENCES SHOULD BE ENABLED
		// If Tecla Access IME is not selected disable all alternative input
		// preferences
	...
	protected void onPause() {
		super.onPause();
		cancelDialog();
		// FIXME: Supposed to force a refresh of preference states, but too
		// aggressive?
		// finish();
	}
	...
	protected void onDestroy() {
		...
		getPreferenceManager().getSharedPreferences()
				.unregisterOnSharedPreferenceChangeListener(TeclaPrefs.this);
	}
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(BluetoothDevice.ACTION_FOUND)
					&& !mShieldFound) {
				BluetoothDevice dev = intent.getExtras().getParcelable(
						BluetoothDevice.EXTRA_DEVICE);
				if ((dev.getName() != null)
						&& (dev.getName().startsWith(
								SwitchEventProvider.SHIELD_PREFIX_2) || dev
								.getName().startsWith(
										SwitchEventProvider.SHIELD_PREFIX_3))) {
					mShieldFound = true;
					mShieldAddress = dev.getAddress();
					mShieldName = dev.getName();
					if (TeclaApp.DEBUG)
						Log.d(TeclaApp.TAG, CLASS_TAG
								+ "Found a Tecla Access Shield candidate");
					cancelDiscovery();
				}
			}
			...
			if (intent.getAction().equals(
				...
				if (mShieldFound) {
					...
					mProgressDialog.setOnCancelListener(null); // Don't do
																// anything if
																// dialog
																// cancelled
					mProgressDialog.setOnKeyListener(new OnKeyListener() {
						...
						public boolean onKey(DialogInterface dialog,
							...
							return true; // Consume all keys once Shield is
											// found (can't cancel with back
											// key)
						}

					});
					mProgressDialog
							.setMessage(getString(R.string.connecting_tecla_shield)
									+ " " + mShieldName);
					if (!SepManager.start(TeclaPrefs.this, mShieldAddress)) {
						// Could not connect to Shield
						dismissDialog();
						Helper.showToast(R.string.couldnt_connect_shield,
								TeclaApp.getInstance());
					}
				} else {
					// Shield not found
					dismissDialog();
					if (!mConnectionCancelled)
						Helper.showToast(R.string.no_shields_inrange,
								TeclaApp.getInstance());
			...
			if (intent.getAction().equals(
					SwitchEventProvider.ACTION_SHIELD_CONNECTED)) {
				if (TeclaApp.DEBUG)
					Log.d(TeclaApp.TAG, CLASS_TAG + "Successfully started SEP");
				dismissDialog();
				Helper.showToast(R.string.shield_connected,
						TeclaApp.getInstance());
			...
			if (intent.getAction().equals(
					SwitchEventProvider.ACTION_SHIELD_DISCONNECTED)) {
				if (TeclaApp.DEBUG)
					Log.d(TeclaApp.TAG, CLASS_TAG + "SEP broadcast stopped");
				dismissDialog();
				mPrefTempDisconnect.setChecked(false);
				mPrefTempDisconnect.setEnabled(false);
			}
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
		...
		if (key.equals(Persistence.PREF_VOICE_INPUT)
				|| key.equals(Persistence.PREF_VARIANTS_KEY)) {
			if (mPrefPersistentKeyboard.isChecked()
					|| mPrefVariantsKey.isChecked()) {
				if (mPrefPersistentKeyboard.isChecked()) {
					// Reset IME
		...
		if (key.equals(Persistence.PREF_SWITCH_J1_TECLA)
				|| key.equals(Persistence.PREF_SWITCH_J1_MORSE)) {
			mSwitchJ1.onPreferenceChanged(key);
			mConfigureInputAdapter.notifyDataSetChanged();
		}
		if (key.equals(Persistence.PREF_SWITCH_J2_TECLA)
				|| key.equals(Persistence.PREF_SWITCH_J2_MORSE)) {
			mSwitchJ2.onPreferenceChanged(key);
			mConfigureInputAdapter.notifyDataSetChanged();
		}
		if (key.equals(Persistence.PREF_SWITCH_J3_TECLA)
				|| key.equals(Persistence.PREF_SWITCH_J3_MORSE)) {
			mSwitchJ3.onPreferenceChanged(key);
			mConfigureInputAdapter.notifyDataSetChanged();
		}
		if (key.equals(Persistence.PREF_SWITCH_J4_TECLA)
				|| key.equals(Persistence.PREF_SWITCH_J4_MORSE)) {
			mSwitchJ4.onPreferenceChanged(key);
			mConfigureInputAdapter.notifyDataSetChanged();
		}
		if (key.equals(Persistence.PREF_SWITCH_E1_TECLA)
				|| key.equals(Persistence.PREF_SWITCH_E1_MORSE)) {
			mSwitchE1.onPreferenceChanged(key);
			mConfigureInputAdapter.notifyDataSetChanged();
		}
		if (key.equals(Persistence.PREF_SWITCH_E2_TECLA)
				|| key.equals(Persistence.PREF_SWITCH_E2_MORSE)) {
		...
		if (key.equals(Persistence.PREF_TEMP_SHIELD_DISCONNECT)) {
			if (mPrefTempDisconnect.isChecked()) {
				...
				{
					if (TeclaApp.DEBUG)
						Log.d(TeclaApp.TAG, CLASS_TAG + "Posted Runnable");
				} else {
					if (TeclaApp.DEBUG)
						Log.d(TeclaApp.TAG, CLASS_TAG
								+ "Could not post Runnable");
				}
		...
		if (key.equals(Persistence.PREF_FULLSCREEN_SWITCH)) {
			if (mPrefFullScreenSwitch.isChecked()) {
				...
				if (!(mPrefSelfScanning.isChecked() || mPrefInverseScanning
						.isChecked())) {
		...
		if (key.equals(Persistence.CONNECT_TO_PC)) {
			if (TeclaApp.mSendToPC && !mConnectToPC.isChecked()) {
				TeclaIME.getInstance().onKey(
						TeclaKeyboardView.KEYCODE_SEND_TO_PC, null);
				// update the send to pc button lock
				TeclaIME.getInstance().onKey(
						TeclaKeyboardView.KEYCODE_SHOW_SECNAV_VOICE, null);
				// TeclaKeyboardView.getInstance().disableSendToPCKey();
				TeclaIME.getInstance().onKey(
						TeclaKeyboardView.KEYCODE_HIDE_SECNAV_VOICE, null);
			}
			TeclaApp.connect_to_desktop = mConnectToPC.isChecked();
			if (!mConnectToPC.isChecked()) {
				setPasswordLaunch.setEnabled(false);
	...
	private void showDiscoveryDialog() {
		...
		mProgressDialog.setOnCancelListener(new OnCancelListener() {
			public void onCancel(DialogInterface arg0) {
				...
				if (TeclaApp.DEBUG)
					Log.d(TeclaApp.TAG, CLASS_TAG
							+ "Tecla Shield discovery cancelled");
				Helper.showToast(R.string.shield_connection_cancelled,
						TeclaApp.getInstance());
				mConnectionCancelled = true;
				mPrefTempDisconnect.setChecked(false);
				mPrefTempDisconnect.setEnabled(false);
				// Since we have cancelled the discovery the check state needs
				// to be reset
	...
	public static void setDefaultSwitchActions() {
		TeclaApp.persistence
				.setFullResetTimeout(Persistence.DEFAULT_FULL_RESET_TIMEOUT);
	...
	public static void createPreferenceDialog(Context context) {
		...
		final EditText psswdtext = (EditText) passworddialog
		...
		psswdtext.setText(setPasswordLaunch.getSharedPreferences().getString(
				Persistence.SET_PASSWORD, "Tecla123"));
		...
		save.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				setPasswordLaunch.getEditor()
						.putString(Persistence.SET_PASSWORD, TeclaApp.password)
						.commit();
	...
	public void showEventSelectionDialog(final int event) {
		...
		final Spinner switchchoice = (Spinner) d
				.findViewById(R.id.spinnerswitch);
		...
		String[] switcharray = { "switch j1/Up", "switch j2/Down",
				"Switch J3/Left", "Switch J4,Right", "Switch E1", "Switch E2" };
		ArrayList<String> switchlist = new ArrayList<String>();
		for (int i = 0; i < switcharray.length; i++)
			switchlist.add(switcharray[i]);
		ArrayAdapter<String> spin1adapter = new ArrayAdapter<String>(
				TeclaPrefs.this, android.R.layout.simple_spinner_item,
				switchlist);
		switchchoice.setAdapter(spin1adapter);

		String[] eventarray = { "onPress", "onRelease", "onClick",
				"onDoubleClick", "onLongPress" };
		ArrayList<String> eventlist = new ArrayList<String>();
		for (int i = 0; i < eventarray.length; i++)
			eventlist.add(eventarray[i]);
		ArrayAdapter<String> spin2adapter = new ArrayAdapter<String>(
				TeclaPrefs.this, android.R.layout.simple_spinner_item,
				eventlist);
			...
			public void onClick(View v) {
				...
				if (event == 0) {
					if (y * 10 + x != TeclaApp.dictation_event) {
						...
						setDisconnectEvent
								.getEditor()
								.putInt("set_disconnect_event",
										TeclaApp.disconnect_event).commit();
						if (TeclaApp.desktop != null
								&& TeclaApp.desktop.isConnected())
							TeclaApp.desktop.send("disevent:"
									+ TeclaApp.disconnect_event);
					} else
						eventchoice
								.setSelection(TeclaApp.dictation_event % 10 - 1);
				} else if (event == 1) {
					if (y * 10 + x != TeclaApp.disconnect_event) {
						TeclaApp.dictation_event = y * 10 + x;
						setDictationEvent
								.getEditor()
								.putInt("set_dictation_event",
										TeclaApp.dictation_event).commit();
						if (TeclaApp.desktop != null
								&& TeclaApp.desktop.isConnected()) {
							TeclaApp.desktop.send("dictevent:"
									+ TeclaApp.dictation_event);
